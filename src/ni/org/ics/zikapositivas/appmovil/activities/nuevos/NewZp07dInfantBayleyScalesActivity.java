...
import ni.org.ics.zikapositivas.appmovil.domain.Zp07dInfantBayleyScales;
import ni.org.ics.zikapositivas.appmovil.parsers.Zp07dInfantBayleyScalesXml;
 ...
 * V1.0
 */
public class NewZp07dInfantBayleyScalesActivity extends AbstractAsyncActivity {
    protected static final String TAG = NewZp07dInfantBayleyScalesActivity.class.getSimpleName();

    private ZikaPosAdapter zikaPos;
    private static Zp07dInfantBayleyScales mInfantAssessment = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        zikaPos = new ZikaPosAdapter(this.getApplicationContext(),mPass,false,false);
        mRecordId = getIntent().getExtras().getString(Constants.RECORDID);
        mInfantAssessment = (Zp07dInfantBayleyScales) getIntent().getExtras().getSerializable(Constants.OBJECTO_ZP07D);
        ...
        if(requestCode == ADD_ZP07_ODK ||requestCode == EDIT_ZP07_ODK) {
            if(resultCode == RESULT_OK) {
                ...
                if (complete.matches("complete")){
                    ...
                    parseZp07dInfantBayleyScales(idInstancia, instanceFilePath, accion);
                }
                else{
    ...
    private void addZp07dInfantBayleyScales() {
        try {
            Uri formUri;
            if (mInfantAssessment == null) {
                ...
                Cursor c = getContentResolver().query(Constants.CONTENT_URI, projection,
                        "jrFormId = 'ZPos07d_Infant_Bayley_Scales' and displayName = 'Zika Positivas ESCALA DE BAYLEY DE DESARROLLO INFANTIL'", null, null);
                c.moveToFirst();
    ...
    private void parseZp07dInfantBayleyScales(Integer idInstancia, String instanceFilePath, Integer accion) {
        Serializer serializer = new Persister();
        File source = new File(instanceFilePath);
        try {
            Zp07dInfantBayleyScalesXml zp07dXml = serializer.read(Zp07dInfantBayleyScalesXml.class, source);
            if (accion== ADD_ZP07_ODK) mInfantAssessment = new Zp07dInfantBayleyScales();
            ...
            try {
                ...
                try {
                    ...
                    if (accionaRealizar == ADD_ZP07_ODK){
                        zikaPos.crearZp07dInfantBayleyScales(mInfantAssessment);
                    }
                    else{
                        zikaPos.editarZp07dInfantBayleyScales(mInfantAssessment);
                    }
                    zikaPos.close();
                } catch (Exception e) {
